...
var peer_list_1 = require("./peer-list");
var express = require("express");
var bodyParser = require("body-parser");
module.exports = function (opts) {
    var router = express.Router();
    router;
    if (opts.peerList && !(opts.peerList instanceof peer_list_1.PeerList)) {
        throw new Error('Invalid peerList');
    }
    // store the peer list on the router
    router.peerList = opts.peerList || new peer_list_1.PeerList();
    ...
    // TODO dig into data
    var sendPeerMessage = function (srcId, destId, data) {
        // find the current peer
        var peer = router.peerList.getPeer(destId);
        if (peer.status()) {
            peer.res
                .status(200)
                .set('Pragma', srcId)
    ...
    router.get('/sign_in', function (req, res) {
        ...
        // add the peer
        var peerId = router.peerList.addPeer(req.query.peer_name, res, req);
        ...
        router.peerList.getPeerIds().filter(function (id) { return id != peerId; }).forEach(function (id) {
    ...
    router.post('/message', bodyParser.text(), bodyParser.urlencoded({ extended: false }), function (req, res) {
        if (!req.query.peer_id ||
            !req.query.to) {
            return res.status(400).end();
        }
        // find the current peer
        var peer = router.peerList.getPeer(req.query.to);
        if (!peer) {
    ...
    router.get('/wait', function (req, res) {
        ...
        var pop = router.peerList.popPeerData(req.query.peer_id);
    ...
    router.get('/sign_out', function (req, res) {
        ...
        // send an updated peer list to all peers
        router.peerList.getPeerIds().forEach(function (id) {
            // updated peer lists must always appear to come from
            // "ourselves", namely the srcId == destId
            sendPeerMessage(id, id, router.peerList.dataFor(id));
        });
